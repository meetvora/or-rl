...
heartbeat_path="${HEARTBEAT_PATH:-logs/evaluate_finetuned_heartbeat.log}"
...
args=(
  -m src.eval
  --model_name_or_path "${MODEL_NAME_OR_PATH:-outputs/rlvr_adapter}" \
  --eval_dir "${EVAL_DIR:-data/eval/complex_or_eval.jsonl}" \
  --output_path "${OUTPUT_PATH:-outputs/finetuned_eval.jsonl}" \
  --responses_path "${RESPONSES_PATH:-outputs/finetuned_eval.responses.jsonl}" \
...
python "${args[@]}" &
eval_pid="$!"
start_heartbeat "$eval_pid" "evaluate_finetuned"
set +e
wait "$eval_pid"
status="$?"
set -e
if [[ -n "$heartbeat_path" ]]; then
  printf '%s label=%s pid=%s event=process_exit exit_code=%s\n' \
    "$(date --iso-8601=seconds)" "evaluate_finetuned" "$eval_pid" "$status" >> "$heartbeat_path"
fi
exit "$status"
